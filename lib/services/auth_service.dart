...
class AuthService {
  ...
  Future<void> initialize() async {
    ...
    if (!hasUsers) {
      ...
      final usersJson = jsonEncode(_predefinedUsers.map((u) => u.toMap()).toList());
  ...
  Future<User?> login(String username, String password) async {
    ...
    for (final user in users) {
      if (user.username.toLowerCase() == username.toLowerCase() && 
        ...
        // Store current user
        await _secureStorage.write(key: _currentUserKey, value: jsonEncode(user.toMap()));
        return user;
      }
    }
  ...
  // Register a new user
  Future<User> register(String username, String password, String fullName, String rank, String department) async {
    final users = await getUsers();
  ...
  Future<bool> enableBiometric() async {
    ...
    try {
      ...
      if (didAuthenticate) {
        ...
        if (userIndex >= 0) {
          users[userIndex] = users[userIndex].copyWith(isBiometricEnabled: true);
          await saveUsers(users);
          
          // Update current user in secure storage
          final updatedUser = currentUser.copyWith(isBiometricEnabled: true);
          await _secureStorage.write(key: _currentUserKey, value: jsonEncode(updatedUser.toMap()));
          
          // Store username for biometric login
          await _secureStorage.write(key: _biometricEnabledKey, value: currentUser.username);
  ...
  Future<User?> loginWithBiometric() async {
    final biometricUsername = await _secureStorage.read(key: _biometricEnabledKey);
    ...
    try {
      ...
      if (didAuthenticate) {
        ...
        final user = users.firstWhere(
          ...
          orElse: () => throw Exception('User not found or biometric not enabled'),
        );
