...
class DatabaseSyncService {
  ...
  final String _baseUrl =
      'https://nafacial-api.example.com/api'; // Replace with actual API endpoint
  ...
  Future<bool> syncDatabase() async {
    ...
    try {
      ...
      // Save the error to shared preferences for debugging
      try {
        final prefs = await SharedPreferences.getInstance();
        ...
        debugPrint('Error saving sync error to preferences: $prefError');
      }
  ...
  Future<void> _savePersonnelToLocalStorage(
      List<Personnel> personnelList) async {
    try {
      final directory = await getApplicationDocumentsDirectory();
      final file = File('${directory.path}/personnel.json');

      // Create a temporary file first to avoid corruption
      final tempFile = File('${directory.path}/personnel.json.tmp');

      try {
        // Write to temp file
        await tempFile
            .writeAsString(
          jsonEncode(personnelList.map((p) => p.toJson()).toList()),
        )
            .timeout(
          const Duration(seconds: 5),
          onTimeout: () {
            throw TimeoutException('Write operation timed out');
          },
        );
        ...
        if (await tempFile.exists() && await tempFile.length() > 0) {
          ...
          await tempFile.rename(file.path).timeout(
            const Duration(seconds: 3),
            onTimeout: () {
              throw TimeoutException('Rename operation timed out');
            },
          );
        } else {
          throw Exception('Temp file was not written correctly');
        }
      } catch (e) {
        ...
        if (await tempFile.exists()) {
          await tempFile.delete().catchError((e) {
            ...
            return tempFile; // Return the file on error
          });
        }
        rethrow; // Re-throw to be caught by outer try-catch
      }
    } catch (e) {
      debugPrint('Error saving personnel to local storage: $e');
      // Try to recover by saving a backup of the data
      try {
        final directory = await getApplicationDocumentsDirectory();
        final backupFile = File('${directory.path}/personnel_backup.json');
        await backupFile.writeAsString(
          jsonEncode(personnelList.map((p) => p.toJson()).toList()),
        );
        debugPrint('Created personnel backup file');
      } catch (backupError) {
        debugPrint('Error creating personnel backup: $backupError');
      }
    }
  }
  ...
  Future<void> _saveChangeLogsToLocalStorage(List<ChangeLog> changeLogs) async {
    try {
      ...
      final newLogs =
          changeLogs.where((log) => !existingIds.contains(log.id)).toList();
      ...
      await file.writeAsString(
          jsonEncode(allLogs.map((log) => log.toJson()).toList()));
    } catch (e) {
      debugPrint('Error saving change logs to local storage: $e');
    }
  }
  ...
  Future<void> addChange({
    ...
    try {
      ...
      debugPrint('Error adding change: $e');
    }
  }
  ...
  Future<void> _addChangeLogToLocalStorage(ChangeLog changeLog) async {
    try {
      ...
      await file.writeAsString(
          jsonEncode(existingLogs.map((log) => log.toJson()).toList()));
    } catch (e) {
      debugPrint('Error adding change log to local storage: $e');
    }
  }
  ...
  Future<void> _addPendingSyncAction(ChangeLog changeLog) async {
    try {
      ...
      debugPrint('Error adding pending sync action: $e');
    }
  }

  // Get change logs for an entity
  Future<List<ChangeLog>> getChangeLogsForEntity(
    ...
    try {
      ...
      debugPrint('Error getting change logs: $e');
      return [];
    }
  }
  ...
  Future<List<ChangeLog>> getAllChangeLogs() async {
    try {
      ...
      debugPrint('Error getting all change logs: $e');
      return [];
    }
  }
